...
def create_expt_folder_with_auto_resuming(OUTPUT_ROOT, name):
    ...
    if os.path.exists(name):
        ...
        print("all existing tags: ", all_existing_tags)
...
class Trainer:
    def __init__(self, config):
        ...
        saved_ckpt = torch.load(os.path.join(config.DATA_ROOT, config.official_ckpt_name))
        _config = saved_ckpt["config_dict"]["_content"]

        self.model = instantiate_from_config(_config['model']).to(self.device)
        self.autoencoder = instantiate_from_config(_config['autoencoder']).to(self.device)
        self.text_encoder = instantiate_from_config(_config['text_encoder']).to(self.device)
        self.diffusion = instantiate_from_config(_config['diffusion']).to(self.device)

        self.autoencoder.load_state_dict(saved_ckpt["autoencoder"])
        self.text_encoder.load_state_dict(saved_ckpt["text_encoder"])
        self.diffusion.load_state_dict(saved_ckpt["diffusion"])

        # load original SD ckpt (with input conv may be modified)
        missing_keys, unexpected_keys = self.model.load_state_dict(saved_ckpt["model"], strict=False)
        assert unexpected_keys == []
        original_params_names = list(saved_ckpt["model"].keys())  # used for sanity check later
        ...
        for name, p in self.model.named_parameters():
            if ("transformer_blocks" in name) and ("rela_fuse" in name):
        ...
        dataset_train = ConCatDataset(config.train_dataset_names, self.data_path, train=True,
    ...
    @torch.no_grad()
    def prepare_relation_phrases(self, batch_prompt, max_relas=5):
        """
        ...
        for prompt in batch_prompt:
            ...
            if "relations" in graph and len(graph["relations"]) > 0:
                ...
                if len(relations) > max_relas:
                    ...
                    _, relation_embeddings = self.text_encoder.encode(relations[:max_relas], return_pooler_output=True)
                else:
                    relation_embeddings = torch.zeros(max_relas, 768).to(self.device)
                    _, _tmp = self.text_encoder.encode(relations, return_pooler_output=True)
                    relation_embeddings[:len(relations), :] = _tmp
            else:
                relation_embeddings = torch.zeros(max_relas, 768).to(self.device)
            relation_embeddings_list.append(relation_embeddings)

        return torch.stack(relation_embeddings_list)
    ...
    def get_input(self, batch):
        ...
        relations = self.prepare_relation_phrases(batch["caption"], self.config.max_relations)
        ...
        return z, t, context, grounding_extra_input, relations

    def run_one_step(self, batch):
        x_start, t, context, grounding_extra_input, relations = self.get_input(batch)
        noise = torch.randn_like(x_start)
        x_noisy = self.diffusion.q_sample(x_start=x_start, t=t, noise=noise)

        grounding_input = self.grounding_tokenizer_input.prepare(batch, self.text_encoder)
