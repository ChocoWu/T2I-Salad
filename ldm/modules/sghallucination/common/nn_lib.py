...
class CategoricalTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # encode the text prompt via CLIPTextModel
    ...
    def forward(
        ...
        if text_prompt is not None:
            text = self.text_prompt_encoder(text_prompt, attention_mask=text_prompt_mask)[0]
        else:
            text = None
        ...
        if self.lookahead:
            ...
            if timestep is not None:
                h = self.backbone(
                    h, src_key_padding_mask=src_key_padding_mask, timestep=timestep, text=text
                )
            else:
                h = self.backbone(h, src_key_padding_mask=src_key_padding_mask, text=text)
        else:
