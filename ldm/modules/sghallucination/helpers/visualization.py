...
def save_each_image(
    ...
    canvas_size: Optional[Tuple[int]] = (260, 240),
    ...
    **kwargs,
    ...
    bbox, label, padding_mask, mask = sparse_to_dense(batch, max_num_nodes=25)

    for i in range(B):
        mask_i = batch_mask[i]
        boxes = batch_boxes[i][mask_i]
        labels = batch_labels[i][mask_i]
        image_name = batch[i].attr['name']
        # base_dir = "/storage_fast/lgqu/generation/dataset/coco/images/val2014"
        # gold_img = Image.open(os.path.join(base_dir, image_name), mode='r')
        # resources = {'img_bg': gold_img}
        pred_img_with_layout = convert_layout_to_image(boxes, labels, colors, canvas_size, names=names, **kwargs)

        # if batch_resources:
        #     resources = {k: v[i] for (k, v) in batch_resources.items()}
        #     img = convert_layout_to_image(
        #         boxes, labels, colors, canvas_size, resources, names, **kwargs
        #     )
        # else:
        #     img = convert_layout_to_image(boxes, labels, colors, canvas_size, resources=resources, names=names, **kwargs)
        each_out_path = os.path.join(result_dir, image_name)
        vutils.save_image(to_tensor(pred_img_with_layout), each_out_path, normalize=False)

        # imgs.append(to_tensor(pred_img_with_layout))
        imgs.append(pred_img_with_layout)

        # save original layout and predicted layout
        g_boxes = bbox[i][mask_i]
        g_labels = label[i][mask_i]
        gold_img_with_layout = convert_layout_to_image(g_boxes, g_labels, colors, canvas_size, names=names, **kwargs)
        g_p_imgs = torch.stack([to_tensor(gold_img_with_layout), to_tensor(pred_img_with_layout)])
        writer.add_images(batch[i].attr['captions'], g_p_imgs)
        # writer.add_image(batch[i].attr['captions'], to_tensor(gold_img_with_layout), 0)
        # writer.add_image(batch[i].attr['captions'], to_tensor(pred_img_with_layout), 1)
        writer.add_text(batch[i].attr['captions'], image_name)
    return imgs
    ...
    **kwargs,
    ...
    for i in range(B):
        ...
        if batch_resources:
            ...
            img = convert_layout_to_image(boxes, labels, colors, canvas_size, names=names, **kwargs)
        each_out_path = os.path.join(result_dir, image_name+'.jpg')
        vutils.save_image(to_tensor(img), each_out_path, normalize=False)
