...
from transformers import AutoProcessor, AutoTokenizer, CLIPModel, CLIPTextModel, CLIPVisionModel
...
def compute_clip_scores(
    ...
    input_ids: [B, S]
    attention_mask: [B, S]
    images: [B], list of Image input
    # average: false -> return the similarity score between each image, true -> return an average sim score within the batch
    Compute the CLIP similarity score between text and image
    """
    device = clip_model.device
    v_inputs = processor(images=images, return_tensors="pt")["pixel_values"].to(device)
    # print(v_inputs.size())
    input_ids = input_ids.to(device)
    attention_mask = attention_mask.to(device)

    outputs = clip_model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=v_inputs)
    cossim = CosineSimilarity(dim=1, eps=1e-6)
    score = cossim(outputs.text_embeds, outputs.image_embeds)

    # v_embds = outputs.
    # logits_per_image = outputs.logits_per_image  # this is the image-text similarity score
    # t_embeds_norm = LA.vector_norm(outputs.text_embeds, dim=1)
    # v_embeds_norm = LA.vector_norm(outputs.image_embeds, dim=1)
    # score = torch.div(logits_per_image, torch.mm(v_embeds_norm.unsqueeze(1), t_embeds_norm.unsqueeze(0)))
    # score = torch.diag(score, 0)

    return {"each": score, "mean": score.mean().item()}


def compute_inception_score(
    imgs: Tensor,
    device: torch.device,
    feature_extractor: Union[str, int, torch.nn.Module] = "logits_unbiased",
    resize: bool = False,
    splits: int = 1
):
    """Computes the inception score of the generated images imgs
    imgs -- batch of 3-channel RGB images of shape (3 x H x W) with dtype uint8
    >>>     from PIL import Image
    >>>     import torchvision.transforms as T
    >>>     #  handle one img
    >>>     img = Image.open('xxx.jpg')
    >>>     img = img.convert('RGB')
    >>>     # np.array(img).shape = [W, H, 3], the value range [0 ~ 255]
    >>>     to_tensor = T.ToTensor()
    >>>     img = to_tensor(img)  # [3, W, H], the value range [0 ~ 1]
    >>>     #  handle multiple image via torch.stack([img, ...])

    cuda -- whether or not to run on GPU
    batch_size -- batch size for feeding into Inception v3
    splits -- number of splits
    resize: Please assure your images are 3x299x299 and if not (e.g. your GAN was trained on CIFAR),
    ...
    if resize:
        imgs = up(imgs)
    inception = IS(feature=feature_extractor, splits=splits)
...
def compute_iou_without_padding(
        box_1: Union[np.ndarray, FloatTensor],
        box_2: Union[np.ndarray, FloatTensor],
        mask: Union[np.ndarray, FloatTensor],
        generalized: bool = False,
):
    iou_s = []
    B = box_1.size(0)
    ...
    for i in range(B):
        ...
        iou_s.append(compute_iou(b1, b2).mean().item())
    # iou_score = torch.stack(iou_s)
    return sum(iou_s) / len(iou_s)
