...
def __compute_maximum_iou_for_layout_qu(layouts_1_layouts_2: Tuple[List[Layout]], ) -> float:
    layout_1, layout_2 = layouts_1_layouts_2
    score = 0.0
    (bi, li), (bj, lj) = layout_1, layout_2
    N = len(bi) # or max(len(bi, bj))? @qu
    for l in list(set(li.tolist())):
        _bi = bi[np.where(li == l)]
        _bj = bj[np.where(lj == l)]
        n = len(_bi)
...
def __get_cond2layouts(layout_list: List[Layout]) -> Dict[str, List[Layout]]:
    ...
    for bs, ls in layout_list:
        cond_key = str(sorted(ls.tolist())) # like: "[56, 57, 58, 60, 62, 63, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73] @qu
...
def compute_maximum_iou_qu(
    ...
    if disable_parallel:
        scores = []
        for arg in args:
            scores.append(__compute_maximum_iou_for_layout_qu(arg))
    else:
        with multiprocessing.Pool(n_jobs) as p:
            scores = p.map(__compute_maximum_iou_for_layout_qu, args)
