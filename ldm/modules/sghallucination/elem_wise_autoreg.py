...
class ElemWiseAutoreg(BaseModel):
    ...
    def __init__(
        ...
        # )
        self.loss_fn_ce = nn.CrossEntropyLoss( ignore_index=self.tokenizer.name_to_id("pad")
        )
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(self.tokenizer.max_token_length):
            with torch.no_grad():
                logits = self.model(input_, text_prompt=text_prompt, text_prompt_mask=text_prompt_mask)["logits"]
    ...
    def preprocess(self, batch):
        ...
        if 'captions' in batch.attr.keys():
            ...
            return {"input": input_, "target": target, "text_ids": text_ids, "text_attention_mask": text_attention_mask}
    ...
    def optim_groups(
        ...
        base = "model.pos_emb"
        additional_no_decay = [
            f"{base}.{name}" for name in self.model.pos_emb.no_decay_param_names
        ]
