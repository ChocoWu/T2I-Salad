...
class BaseMaskAndReplaceDiffusion(torch.nn.Module):
    ...
    def predict_start(self, log_x_t, t, text_prompt=None, text_prompt_mask=None):  # p(x0|xt)
        x_t = log_onehot_to_index(log_x_t)
        out = self.transformer(x_t, timestep=t, text_prompt=text_prompt, text_prompt_mask=text_prompt_mask)["logits"]
    ...
    def _sample_single_step(
        ...
        with torch.no_grad():
            # Infer x0 at first
            log_x_recon = self.predict_start(log_z, model_t, text_prompt=text_prompt, text_prompt_mask=text_prompt_mask)
