...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, relations=None, boxes=None, masks=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, objs, relations, boxes, masks)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.sg_encoder = instantiate_from_config(sg_encoder)  # for modeling generated sg 
    ...
    def forward(self, input):
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, objs, relations, boxes, masks)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, relations, boxes, masks)
