...
class HierachicalSceneIntegration(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, **kwargs):
        super().__init__()
    ...
    def forward(self, x, objects, relations, regions, globals):
        pass


class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=False):
        super().__init__()
    ...
    def forward(self, x, context, objs, relations=None, boxes=None, masks=None, h=None, w=None):
        return self._forward(x, context, objs, relations, boxes, masks, h, w)

    def _forward(self, x, context, objs, relations=None, boxes=None, masks=None, h=None, w=None):
        x = self.attn1(self.norm1(x)) + x
        x = self.fuser(x, objs)  # identity mapping in the beginning
        # print("before add relation interaction:", torch.var(x))
        # x = (self.rela_fuse(x, relations, boxes, masks, h, w) + x) / 2
        # print("after add relation interaction: ", torch.var(x))
...
class SpatialTransformer(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None,
        ...
        #                                       stride=1,
        #                                       padding=0))
        self.proj_out = nn.Conv2d(query_dim,
                                  in_channels,
                                  kernel_size=1,
                                  stride=1,
                                  padding=0)

    def forward(self, x, context, objs, relations=None, boxes=None, masks=None):
        b, c, h, w = x.shape
        x_in = x
        x = self.norm(x)
        x = self.proj_in(x)
        ...
        for block in self.transformer_blocks:
            x = block(x, context, objs, relations, boxes, masks, h, w)
