...
def build_vocab(data_list):
    """
    Build a object category, attribute category, and relation predicate vocabulary from the given data list.
    """
    pass


def build_embedding(data_list):
    """
    Build a embedding matrix of object category, attribute category, and relation predicate embedding from the given data list.
    Therefore, we can use utilize the embedding for the object category, attribute category, and relation predicate to pretrain the models.
    """
    pass
...
class SgDataset(BaseDataset):
    def __init__(self,
        ...
        self.region_description_path = region_description_path
        ...
        objects_data = read_json(self.object_path)
        image_data = vg.get_all_image_data()
        
        attributes_data = read_json(self.attribute_path)
        relations_data = read_json(self.relation_path)
        region_description_data = read_json(self.region_description_path)
        ...
        # first obtain the imageid—to-objects
        imageId2objects = defaultdict(list)
        for objects in objects_data:
            imageId2objects[objects["image_id"]].append(objects)

    def preprocess(self):

        """
        Preprocess the data and save the preprocessed data to the preprocess_data
        """
